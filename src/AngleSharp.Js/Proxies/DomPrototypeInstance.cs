...
namespace AngleSharp.Js
{
    ...
    sealed class DomPrototypeInstance : ObjectInstance
    {
        ...
        public Boolean TryGetFromIndex(Object value, String index, out PropertyDescriptor result)
        {
            ...
            if (_stringIndexer != null && !TryGetProperty(index, out _))
            {
        ...
        private void SetMethod(String name, MethodInfo method)
        {
            ...
            var propertyName = new JsString(name);
            if (!TryGetProperty(propertyName, out _))
            {
                var func = ObjectWrapper.GetPropertyDescriptor(_instance.Jint, this, method);
                SetOwnProperty(propertyName, func);
            }
        }
    }
}
